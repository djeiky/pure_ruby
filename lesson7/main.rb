...
def show_train_carriges(train = nil)
  ...
  if train.type == 'Passenger'
    train.show_carriges {|c| puts "Carrige number - #{c.number} passengers - #{c.passengers_count} free seats - #{c.free_seats}"}
  else
    train.show_carriges {|c| puts "Carrige number - #{c.number} weight - #{c.weight} remain weight - #{c.remain_weight}"}
  end
end
  ...
  @stations.each_with_index do |val, index| 
    ...
    val.show_trains do |train| 
      ...
      train.carriges.each do|c|
        ...
        if c.is_a? PassengerCarrige
          str += "taken seats - #{c.passengers_count} free seats #{c.free_seats}"
        else
          str += "weight - #{c.weight} remain weight - #{c.remain_weight}"
        end
        puts str
      end
    end
